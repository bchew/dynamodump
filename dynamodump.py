...
def do_backup(conn, table_name, read_capacity):
  ...
  if not args.schemaOnly:
    ...
    logging.info("Schema dump for " + table_name + " table completed. Time taken: " + str(datetime.datetime.now().replace(microsecond=0) - start_time))
...
parser.add_argument("-p", "--profile", help="AWS profile name to use. Allows to you to choose alternate profile from your AWS credentials file.")
...
parser.add_argument("--schemaOnly", action="store_true", default=False, help="Backup or restore the schema only. Do not export/import data. [optional]")
