...
def do_restore(dynamo, sleep_interval, source_table, destination_table, write_capacity):
    ...
    if not args.schemaOnly:
        ...
        # read data files if data dir exists
        if not os.path.exists(dump_data_path + "/" + source_table + "/" + DATA_DIR + "/"):
            logging.info("Data directory for " + destination_table + " missing. Skipping data restore..")
        else:
            # read data files
